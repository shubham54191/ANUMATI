...
export function DecisionBar({ app }: { app: ApplicationFile }) {
  ...
  return (
    <div className="flex-none border-t border-line bg-surface px-5 py-3">
      <div className="flex flex-wrap items-center gap-3">
        <Label className="flex-none">Departmental desks</Label>

        <div className="flex flex-wrap items-center gap-1.5">
          {openReviews.length === 0 ? (
            <span className="text-[12px] text-muted">Every desk on this file has decided.</span>
          ) : null}
          {openReviews.map((r) => (
            <button
              key={r.dept_id}
            ...
            >
              {r.dept_short}
              <span className="text-[9.5px] text-faint">{r.approval_id}</span>
            </button>
          ))}
        </div>

        {selected ? (
          <div className="flex items-center gap-2 rounded border border-line bg-sunk px-2 py-1.5">
            ...
            {weighted ? (
              <label className="flex items-center gap-1.5">
                ...
                <input
                  ...
                  className="h-7 w-[68px] rounded border border-control bg-surface px-2 font-mono text-[11.5px] text-ink outline-none focus-visible:ring-2 focus-visible:ring-accent"
                />
              </label>
            ) : null}
            <Button onClick={() => act("approved")} className="h-7 px-2.5">
              <Check className="h-3 w-3 text-state-done-ink" strokeWidth={2} />
              Approve
            </Button>
            <Button onClick={() => act("rejected")} className="h-7 px-2.5">
              <X className="h-3 w-3 text-critical" strokeWidth={2} />
              Reject
            </Button>
          </div>
        ) : null}
