    ...
    <div className="rounded border border-line bg-surface">
      ...
      <div className="flex items-end gap-4 px-3.5 pt-3">
        <div>
          <span
            className={cn(
              "font-num font-serif text-[38px] font-medium leading-none",
              pass ? "text-state-done-ink" : "text-critical",
            )}
          >
            {score.toFixed(1)}
          </span>
          <span className="ml-1 text-[12px] text-muted">/ 100</span>
        </div>
        <div className="pb-1">
          <div className="font-mono text-[11px] text-muted">
            passing score <span className="font-semibold text-ink">{threshold}</span>
          </div>
          <div
            className={cn(
      ...
      <div className="px-3.5 pb-3 pt-2.5">
        <div className="relative h-[10px] overflow-hidden rounded-sm bg-sunk">
          <div
            className={cn("h-full rounded-sm transition-[width] duration-500", pass ? "bg-state-done" : "bg-critical")}
        ...
        <div className="relative mt-1 h-4">
          <span
            className="absolute -translate-x-1/2 font-mono text-[9.5px] tracking-[0.05em] text-ink"
            style={{ left: `${threshold}%` }}
          >
            {threshold} PASS
          </span>
        ...
        <div className="flex flex-col gap-1.5">
          {contributions.map((c) => (
            <div key={c.dept_short} className="flex items-center gap-2.5">
              <span className="w-[76px] flex-none font-mono text-[11px] text-ink">{c.dept_short}</span>
              <div className="h-[5px] flex-1 overflow-hidden rounded-sm bg-sunk">
                <div
                  className={cn("h-full rounded-sm", c.score >= threshold ? "bg-state-done" : "bg-critical")}
                  style={{ width: `${c.score}%` }}
                />
              </div>
              <span className="font-num w-[64px] flex-none text-right font-mono text-[11px] text-ink">
                {c.score}/100
              </span>
              <span className="font-num w-[62px] flex-none text-right font-mono text-[10.5px] text-faint">
